...
errors = []
warnings = []
required = ["slug", "version", "snapshot_date", "topic", "sources"]
for field in required:
    if field not in data:
        errors.append(f"missing required field: {field}")

# cogworks_version is recommended for generated skills but not required on source skills
if "cogworks_version" not in data:
    warnings.append("missing recommended field: cogworks_version")
...
PYEOF
    ...
    if [[ "$validation_result" == "OK" ]]; then
        ...
        while IFS= read -r line; do
            if [[ "$line" == ERROR:* ]]; then
                log_critical "metadata.json: ${line#ERROR:}"
                has_errors=true
            elif [[ "$line" == WARN:* ]]; then
                log_warning "metadata.json: ${line#WARN:}"
            fi
        done <<< "$validation_result"
        if $has_errors; then
            return 1
        fi
        log_pass "metadata.json valid (with warnings)"
    fi
}
