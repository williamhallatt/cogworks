...
If user invoked with `--test` flag (e.g., `@cogworks encode <sources> --test`):

1. **Invoke cogworks-test skill**:
   ```
   /cogworks-test {slug}
   ```
...
3. **Receive validation report** with:
   - Overall score and recommendation (PASS/FAIL)
   - Category scores (source fidelity, self-sufficiency, etc.)
   - Critical failures (if any)
   - Warnings (if any)
   - Detailed validation report path
...
4. **Handle results**:
   ...
   **If critical failures detected**:
   ...
   - If user chooses to fix: apply fixes and re-run validation
   - If user chooses to proceed: continue to Step 7 with warning

   **If warnings only**:
   - Include validation summary in Step 7 confirmation
   - Note warnings but mark as PASSED
   - Continue to Step 7

   **If all tests passed**:
   - Include test score in Step 7 confirmation
   - Continue to Step 7

5. **Log test results**: Save to `tests/results/{timestamp}/` for future reference

**Note**: Testing is opt-in via --test flag. If flag not provided, skip this step entirely.
...
- **If tested**: Include validation summary (score, PASS/FAIL, warnings)

## Edge Case Handling

- **Insufficient or sparse sources** - If the provided material is too sparse to meet synthesis targets (e.g., fewer than 5 concepts extractable), produce the best synthesis possible, explicitly state what is thin, and ask the user whether to proceed with reduced coverage or provide additional sources. If --test flag is used, automated testing will also detect and warn about insufficient sources.
...
- **Overlapping with built-in knowledge** - If source material contains only generic information that Claude already knows (e.g., "write clear code"), suggest reconsidering whether a skill is needed. If --test flag is used, automated testing will detect low novelty and recommend reconsidering.
...
4. **If --test flag used**: Automated testing completed (optional)
