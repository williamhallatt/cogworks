...
usage() {
  cat <<'EOF' >&2
Usage: scripts/behavioral-capture-codex.sh <skill_slug> <case_id> <case_json_path> <raw_trace_path>

Required environment:
  COGWORKS_BEHAVIORAL_CODEX_REAL_CMD

Template placeholders supported in COGWORKS_BEHAVIORAL_CODEX_REAL_CMD:
...
if [[ -z "$template" ]]; then
  echo "COGWORKS_BEHAVIORAL_CODEX_REAL_CMD is required." >&2
  echo "Example: export COGWORKS_BEHAVIORAL_CODEX_REAL_CMD=\"my-codex-capture --skill '{skill_slug}' --case '{case_id}' --case-json '{case_json_path}' --out '{raw_trace_path}'\"" >&2
  exit 2
fi
