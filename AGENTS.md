...
- `plugin/` contains the Claude Code plugin package: `plugin/.claude-plugin/plugin.json` (manifest), `plugin/skills/` (rendered distribution skills), `plugin/agents/` (rendered Claude Code format agents).
...
- **`.claude/agents/` is auto-loaded by Claude Code** — any file edited there immediately changes the instructions for every active Claude Code session in this repo. These are rendered artifacts; edit source in `agents/` or `skills/cogworks/role-profiles.json` and re-run `scripts/render-agentic-role-bindings.py`.
