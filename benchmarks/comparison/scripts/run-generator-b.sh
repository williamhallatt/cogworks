...
sources_path="$1"
out_dir="$2"
comparator_dir="$ROOT_DIR/benchmarks/comparison/comparators/generator-b"
raw_dir="$out_dir/comparator-raw"
mkdir -p "$raw_dir"

template="${COGWORKS_BENCH_GENERATOR_B_CMD:-}"
...
if [[ -z "$template" ]]; then
  if [[ -x "$default_script" ]]; then
    ...
    echo "Missing comparator runner for generator-b." >&2
    echo "Provide COGWORKS_BENCH_GENERATOR_B_CMD or executable $default_script" >&2
    exit 2
  fi
fi
