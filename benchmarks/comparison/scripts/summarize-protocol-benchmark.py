...
def render_report(summary):
    ...
    lines.append("| Rank | Pipeline | Mean Quality | Valid Tasks |")
    lines.append("|---:|---|---:|---:|")
    for i, row in enumerate(summary["aggregate_ranking"], start=1):
        lines.append(f"| {i} | {row['pipeline']} | {row['mean_quality']:.4f} | {row['valid_tasks']} |")
    lines.append("")

    lines.append("## Per-Task Ranking")
    for task in summary["task_rankings"]:
        lines.append(f"### {task['task_id']}")
        lines.append("| Rank | Pipeline | Quality | Valid | Reason |")
        lines.append("|---:|---|---:|---|---|")
        for i, row in enumerate(task["ranking"], start=1):
            lines.append(f"| {i} | {row['pipeline']} | {row['quality']:.4f} | {row['valid']} | {row['reason']} |")
        lines.append("")
...
def main():
    ...
    p.add_argument("--results-root", default="benchmarks/comparison/results/pipeline-benchmark")
    ...
    # Also emit a quality-first ranking file for consistency with other benchmark flows.
