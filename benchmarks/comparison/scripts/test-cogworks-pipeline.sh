...
Options:
  ...
  --manifest <path>       Manifest path (default: benchmarks/comparison/datasets/pipeline-benchmark/manifest.jsonl)
...
python3 "$CLI" pipeline-benchmark run \
  ...
  "${FORCE_ARGS[@]}" \
  "${RUN_ARGS[@]}" \
  --command-template "claude::$ROOT_DIR/benchmarks/comparison/scripts/run-benchmark.sh claude '{sources_path}' '{out_dir}'" \
  --command-template "codex::$ROOT_DIR/benchmarks/comparison/scripts/run-benchmark.sh codex '{sources_path}' '{out_dir}'"
